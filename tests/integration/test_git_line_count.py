...
async def test_debug_git_line_counts():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = subprocess.run(["git", "diff", "--cached", "--numstat"], cwd=repo_path, capture_output=True, text=True)
        ...
        result = subprocess.run(["git", "status", "--porcelain", "--", "test_file.py"], cwd=repo_path, capture_output=True, text=True)
        ...
        if len(file_status) >= 2:
            ...
            assert working_status in [None, " "], f"Expected no working status but got '{working_status}'"
        ...
        try:
            ...
            for staged_param in [True, False, None]:
                try:
                    ...
                    if staged_param in [True, None]:  # Should work for staged files
                        assert diff_stats["lines_added"] == 1, f"Expected 1 line added but got {diff_stats['lines_added']} with staged={staged_param}"
                        assert diff_stats["lines_deleted"] == 0, f"Expected 0 lines deleted but got {diff_stats['lines_deleted']} with staged={staged_param}"
            ...
            repo_model = LocalRepository(path=repo_path, name="test_repo", current_branch="main", head_commit="test")
