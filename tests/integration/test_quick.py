...
def print_dict(data, indent=0):
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            elif isinstance(value, list) and key not in ["recommendations", "files_affected", "potential_conflict_files", "high_risk_files", "factors", "large_changes", "warnings", "errors", "action_plan"]:
                 print(f"{'  ' * indent}{key}:")
                 for item in value:
...
async def test_analyze_repo_with_unstaged_changes(tmp_path):
    ...
    async with client:
        ...
        raw_summary_result = await client.call_tool("get_outstanding_summary", {"repository_path": str(repo_path), "detailed": True})
        ...
        assert summary_data["quick_stats"]["staged_changes"] == 0 # Key assertion for unstaged
        ...
        raw_wd_result = await client.call_tool("analyze_working_directory", {"repository_path": str(repo_path), "include_diffs": False})
        wd_data = _extract_tool_data(raw_wd_result)
        print_result(wd_data)
        assert isinstance(wd_data, dict)
        modified_files = wd_data["repository_status"]["working_directory"]["modified_files"]
        
        assert len(modified_files) == 1, f"Expected 1 modified file in working directory, got {len(modified_files)}"
        assert modified_files[0]["path"] == "test_file.py", f"Expected 'test_file.py', got {modified_files[0]['path']}"
        assert modified_files[0].get("lines_added", 0) == 1, f"Expected 1 line added in WD, got {modified_files[0].get('lines_added')}"
        assert not modified_files[0]["staged"], "Expected file to be NOT staged"
        assert wd_data["repository_status"]["working_directory"]["total_files"] == 1, "Expected total_files in WD to be 1"
        ...
        raw_staged_result = await client.call_tool("analyze_staged_changes", {"repository_path": str(repo_path)})
        staged_data = _extract_tool_data(raw_staged_result)
        print_result(staged_data)
        assert isinstance(staged_data, dict)
        assert staged_data.get("total_staged_files") == 0, "Expected 0 staged files for unstaged scenario"
        assert staged_data.get("ready_to_commit") is False, "Expected not ready to commit for unstaged scenario"
        ...
        raw_health_result = await client.call_tool("analyze_repository_health", {"repository_path": str(repo_path)})
        health_data = _extract_tool_data(raw_health_result)
        print_result(health_data)
        assert isinstance(health_data, dict)
        assert health_data.get("health_score", 100) < 100, "Expected health score less than 100 due to unstaged changes"
        assert health_data.get("health_status") == "good" or health_data.get("health_status") == "fair", "Expected health status to reflect changes"
        assert "Uncommitted changes in working directory" in health_data.get("issues", []), "Expected 'Uncommitted changes' issue"
        print("✅ Repository Health (Unstaged) check passed.")

    print(f"\n" + "="*80)
    print(f"FINISHED TEST: UNSTAGED CHANGES SCENARIO")
    print("="*80 + "\n")
...
async def test_analyze_repo_with_staged_changes(tmp_path):
    ...
    subprocess.run(["git", "init"], cwd=repo_path, check=True)
    subprocess.run(["git", "add", "test_file.py"], cwd=repo_path, check=True)
    subprocess.run(["git", "commit", "-m", "Initial commit for staged test"], cwd=repo_path, check=True)
    ...
    async with client:
        ...
        assert isinstance(summary_data, dict)
        assert summary_data.get("has_outstanding_work") is True
        assert summary_data["quick_stats"]["working_directory_changes"] == 0 # Key assertion for staged
        ...
        print_result(wd_data)
        assert isinstance(wd_data, dict)
        assert wd_data["repository_status"]["working_directory"]["total_files"] == 0, "Expected 0 modified files in working directory for staged scenario"
        ...
        assert staged_files[0]["path"] == "test_file.py", f"Expected 'test_file.py', got {staged_files[0]['path']}"
        assert staged_files[0].get("lines_added", 0) == 1, f"Expected 1 line added in staged, got {staged_files[0].get('lines_added')}"
        ...
        print_result(health_data)
        assert isinstance(health_data, dict)
        assert health_data.get("health_score", 100) < 100, "Expected health score less than 100 due to staged changes"
        assert health_data.get("health_status") == "good" or health_data.get("health_status") == "fair", "Expected health status to reflect changes"
        assert health_data["metrics"]["has_staged_changes"] is True, "Expected has_staged_changes to be True"
        print("✅ Repository Health (Staged) check passed.")

    print(f"\n" + "="*80)
    print(f"FINISHED TEST: STAGED CHANGES SCENARIO")
    print("="*80 + "\n")
